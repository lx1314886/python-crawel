...
class JxhjSpider():
    ...
    def parse(self):
        response = requests.get('https://www.smzdm.com/jingxuan/', headers=self.headers).text
        sel = Selector(text=response)
        jx_list = sel.xpath(contents.XPATH_BOOT)
        jx_result = []
        for jx in jx_list:
            item = {}
            item['lTitle'] = jx.xpath(contents.XPATH_L_TITLE).extract_first().replace("'",'')
            item['lImageUrl'] = jx.xpath(contents.XPATH_L_IMAGE_URL).extract_first()
            item['vouchers'] = jx.xpath(contents.XPATH_QUAN).extract_first()
            item['descr'] = jx.xpath(contents.XPATH_CONTENTS).extract_first().replace("'",'')
            if len(item['descr']) == 0:
                item['descr'] = jx.xpath(contents.XPATH_CONTENTS_).extract_first().replace("'", '')
            goUrl = jx.xpath(contents.XPATH_GO_URL).extract_first()
            if goUrl is None:
                continue
            item['titleUrl'] = goUrl
            # 正则匹配商品编码
            shopNum = re.search(r'-?\d+', str(goUrl), re.M|re.I).group()
            print(shopNum)
            item['shopNum'] = shopNum
            item['goShoppingUrl'] = jx.xpath(contents.XPATH_GO_SHOPPING_URL).extract_first()
            if item['goShoppingUrl'] is None:
                item['goShoppingUrl'] = jx.xpath(contents.XPATH_GO_SHOPPING_URL_).extract_first()
            if item['goShoppingUrl'] is None:
                item['goShoppingUrl'] = jx.xpath(contents.XPATH_GO_SHOPPING_URL_1).extract_first()
            if item['goShoppingUrl'] is None:
                continue
            ...
            time1 = jx.xpath(contents.XPATH_TIME).extract_first()
            if len(time1.split( )) > 2:
                item['time1'] = time1.split( )[0] + ' ' + time1.split()[1]
                item['jxuan'] = time1.split( )[2]
            else:
                item['time1'] = time1.split( )[0]
                item['jxuan'] = time1.split( )[1]
            item['pageUrl'] = goUrl
            self.parse_detail(item)
            jx_result.append(item)
        if len(jx_result) > 0:
            with UsingMysql1(log_time=True) as um:
                # 删除数据
                del_data = [(jx_result[i]['shopNum']) for i in range(len(jx_result))]
                ...
                smzdm_data = [(jx_result[i]['lTitle'],jx_result[i]['titleUrl'],jx_result[i]['descr']
                                                                                ,jx_result[i]['goShoppingUrl'],jx_result[i]['time1'],jx_result[i]['lImageUrl']
                                                                                ,jx_result[i]['vouchers'],jx_result[i]['sType'],jx_result[i]['jxuan'],jx_result[i]['createDate']
                                                                                ,jx_result[i]['grabTime'],jx_result[i]['shopNum']) for i in range(len(jx_result))]
                um.update_batch_by_pk(contents.SMZDM_DATA_SQL, smzdm_data)
                smzdm_data_detail = [(jx_result[i]['dTitle'], jx_result[i]['contents'], jx_result[i]['quan'], jx_result[i]['dImageUrl'], jx_result[i]['pageUrl'], jx_result[i]['createDate'], jx_result[i]['createDate'], jx_result[i]['shopNum']) for i in range(len(jx_result))]
    ...
    def parse_detail(self, item):
        ...
        if len(couponContent) != 0 and len(couponUrl) != 0:
